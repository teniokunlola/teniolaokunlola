...
import { Eye, EyeOff, Lock, Mail, Shield, Sparkles, ArrowRight, Sun, Moon } from 'lucide-react';
import { motion } from 'framer-motion';
import { useUIStore, useThemeSync } from '@/store';
import { validateAndSanitizeEmail, validateAndSanitizePassword, RateLimiter } from '@/lib/security';
...
const Login: React.FC = () => {
  ...
  const rateLimiter = React.useMemo(() => new RateLimiter(5, 300000), []); // 5 attempts per 5 minutes
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // 1. Sign in with Firebase with sanitized data
      const userCred = await signInWithEmailAndPassword(auth, emailValidation.value, passwordValidation.value);
      const idToken = await userCred.user.getIdToken();
      // 2. Fetch admin profile from backend
      const res = await fetch(buildApiUrl('current-admin-user/'), {
        headers: { Authorization: `Bearer ${idToken}` },
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Not an admin or invalid credentials.');
      ...
      logger.error('Login failed', { error: e, email });
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex items-center justify-center p-4 relative overflow-hidden">
      {/* Floating particles - Same as Home.tsx */}
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  onChange={e => {
                    ...
                    if (value.length <= 254) { // Email length limit
                      setEmail(value);
                    }
                  }}
                  required
            ...
            >
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  maxLength={128}
                  disabled={loading}
                  className="pl-10 pr-12 bg-background border-border text-foreground placeholder-muted-foreground focus:border-purple-500 focus:ring-purple-500/20 transition-all duration-300"
                  placeholder="Enter your password"
                />
                <motion.button
                  type="button"
                  tabIndex={-1}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-purple-500 dark:text-purple-400 hover:text-purple-600 dark:hover:text-purple-300 transition-colors duration-200"
